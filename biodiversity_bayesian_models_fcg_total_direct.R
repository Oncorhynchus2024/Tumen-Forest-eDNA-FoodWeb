...
#   env.txt
#     Required columns: site, river, FCG, river_width
#   spe_bacteria.txt
#   spe_protist.txt
#   spe_Metazoa.txt
#   spe_fish.txt
...
int_terms <- c("int_FCG_mainstem", "int_FCG_river_width")
...
compute_site_beta_local <- function(X_asv, env2,
  ...
  meta <- env2[keep, c("site","river","FCG","river_width","mainstem"), drop=FALSE]
  ...
  for(rv in rivers){
    ...
    for(cm in comb_list){
      ...
      fi_mean <- mean(sub_meta$FCG, na.rm=TRUE)
...
if(!"FCG" %in% names(env) && "fi" %in% names(env)) env$FCG <- env$fi
if(!"FCG" %in% names(env)) stop("env.txt must contain 'FCG' column.")
...
env_num$int_FCG_mainstem    <- env_num$FCG * env_num$mainstem
env_num$int_FCG_river_width <- env_num$FCG * env_num$river_width
...
run_one_group <- function(prefix, spe_file, env_num, model_type){
  ...
  tryCatch({
    ...
    if(!("FCG" %in% preds_all)) stop("FCG not in predictors after filtering (unexpected).")
    ...
    if(!(paste0(focus_main, "_z") %in% Z_names_a)) stop("FCG removed by collinearity filtering (unexpected).")
...
"
    ...
    grid::grid.draw(gridExtra::arrangeGrob(
      gridExtra::arrangeGrob(
        grid::textGrob(
          paste0(prefix, " HOP (", model_type, "): FCG -> Shannon / Evenness / Beta_local"),
    ...
    grid::grid.draw(gridExtra::arrangeGrob(
      grid::textGrob(
        paste0(prefix, " PDP (", model_type, "): FCG × other = ", other_alpha),
        gp=grid::gpar(fontsize=14, fontface="bold", fontfamily=PLOT_FAMILY)
      ),
    ...
    grid::grid.draw(gridExtra::arrangeGrob(
      grid::textGrob(
        paste0(prefix, " PDP(withInt) (", model_type, "): FCG × other = ", other_alpha),
        gp=grid::gpar(fontsize=14, fontface="bold", fontfamily=PLOT_FAMILY)
      ),
      gridExtra::arrangeGrob(grobs=list(p_pdp_shan, p_pdp_even, p_pdp_beta), ncol=3),
      ncol=1, heights=c(0.15, 0.85)
