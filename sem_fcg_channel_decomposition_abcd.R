...
#
# This version focuses on FCG-plus-channel decomposition and AB/CD figure generation.
...
OUT_DIR <- file.path(getwd(), "SEM_Output_3Med_4Shan_Controlled_HideWidth_StrictPrior_Cut020")
...
write_method_note <- function(out_dir){
    txt <- c(
        ...
        "7) CD figures show FCG full effect plus FCG-centered channel decomposition.",
...
fi_channels <- c("tsm", "nutrient_pc1", "physchem_pc1", "bacteria_shannon", "protist_shannon", "metazoa_shannon", "fish_shannon")
fi_effects_comp <- calc_fi_channel_effects_posterior(
    edge_draws = comp_res$draws,
    fi_node    = "FCG",
    channels   = fi_channels,
    endpoint   = "trophic_incoherence",
    label_map  = LABEL_MAP
)

fi_full_comp <- calc_full_cascading_effects_posterior(
    edge_draws = comp_res$draws,
    preds      = "FCG_z",
...
effects_comp_mixed <- bind_rows(
    fi_full_comp$summary_long %>% mutate(Predictor = "FCG"),
    fi_effects_comp$summary_long %>% filter(Predictor != "FCG overall")
) %>%
    mutate(
        Predictor = factor(
            Predictor,
            levels = c(
                "FCG",
                "FCG via TSM",
                "FCG via Nutrient PC1",
                "FCG via PhysChem PC1",
                "FCG via Bacteria H'",
                "FCG via Protist H'",
                "FCG via Metazoa H'",
                "FCG via Fish H'"
...
fi_effects_struct <- calc_fi_channel_effects_posterior(
    edge_draws = struct_res$draws,
    fi_node    = "FCG",
    channels   = fi_channels,
    endpoint   = "niche_overlap",
    label_map  = LABEL_MAP
)

fi_full_struct <- calc_full_cascading_effects_posterior(
    edge_draws = struct_res$draws,
    preds      = "FCG_z",
...
effects_struct_mixed <- bind_rows(
    fi_full_struct$summary_long %>% mutate(Predictor = "FCG"),
    fi_effects_struct$summary_long %>% filter(Predictor != "FCG overall")
) %>%
    mutate(
        Predictor = factor(
            Predictor,
...
write.table(
  effects_comp_mixed,
  file = file.path(OUT_DIR, "Composition_FIplusChannel_Effects.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)
...
write.table(
  ...
  file = file.path(OUT_DIR, "Structure_FIplusChannel_Effects.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)
...
# -----------------------------
# 14) Calculate Global Y-Limits & Plot (Row 2)
# -----------------------------
...
p_comp_eff <- plot_effect_sizes_fi_channels(
  effects_comp_mixed,
  title = "C. FCG and FCG-channel effects on Trophic Incoherence",
  ylim_range = y_limits_global
)

p_struct_eff <- plot_effect_sizes_fi_channels(
  effects_struct_mixed,
  title = "D. FCG and FCG-channel effects on Niche Overlap",
  ylim_range = y_limits_global
)
...
out_combined_pdf <- file.path(OUT_DIR, "Fig_Integrated_SEM_Controlled_HideWidth_StrictPrior_Cut020_FIplusChannels.pdf")
